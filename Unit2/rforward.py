...
def handler(chan, host, port):
    """
        功能： 连接目标地址：端口 并且 接收消息
    """
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    try:
        sock.connect((host, port))
        verbose('Success to connect to {}:{}.'.format(host, port))
    except Exception as e:
        verbose('Forwarding request to {}:{} failed:{}'.format(host,port,e))
        return

    verbose('Connected! Tunnel open {} -> {} -> {}'.format(chan.origin_addr, chan.getpeername(), (host, port)))
...
def reverse_forward_tunnel(server_port, remote_host, remote_port, transport):
    """
        这里使用paramiko的 request_post_forward 函数将SSH服务端的一个端口的TCP连接转发出去
    """
...
def parse_options():
    ...
    parser.add_option('-p', '--remote-port', action='store', type='int', dest='port',
                      default = DEFAULT_PORT, help = 'port on server to forward (default: %d)' % DEFAULT_PORT)
...
def main():
    ...
    verbose('Now forwarding remote port {} to {}:{}...'.format(options.port, remote[0], remote[1]))
